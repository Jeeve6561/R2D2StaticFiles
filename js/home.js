...
const DocElems = {
  ...
  writetodb: document.getElementById("writetodb"),
};
...
const LiveFeedWS = {
  ...
  messageReceived(event) {
    ...
    // console.log("This read took", performance.now() - Constants.testTime);
    // Constants.testTime = performance.now();
    // console.log(
    //   event.data.length, "bytes has performance of database write:",
    //   performance.now() - Constants.PerformanceTest
    // );
    // Constants.PerformanceTest = performance.now();
  },
...
function CreateAndInitAllDatabases(stores) {
  ...
  openRequest.onupgradeneeded = function (event) {
    ...
    stores.forEach((sym) => {
      ...
      store.createIndex(Constants.Database.Store.keyPath, Constants.Database.Store.keyPath);
    });
